...
history = model.fit([X_train_all, X_train_time, X_train_temp], y_train,
                    validation_data=([X_test_all, X_test_time, X_test_temp], y_test), epochs=50, verbose=1, shuffle=False)

# Evaluate the model on the training data
loss, rmse = model.evaluate([X_train_all, X_train_time, X_train_temp], y_train, verbose=0)
print('-----training data-----')
print(f'loss: {loss}')
print(f'rmse: {rmse}')

# Evaluate the model on the testing data, kernel_regularizer=regularizers.l2(0.01)
loss, rmse = model.evaluate([X_test_all, X_test_time, X_test_temp], y_test, verbose=0)
print('-----testing data-----')
print(f'loss: {loss}')
print(f'rmse: {rmse}')
...
predictions_total = [y1 + y2 for y1, y2 in predictions]
