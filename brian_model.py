...
class BrianModel(NeuralNetworkModel):
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.DataFrame) -> None:
        # return super().run_model(X_train, y_train)
        ...
        model = Sequential()  # Sequential just means the network doesn't have loops--the outputs of one layer of
        # neurons go to the next layer of neurons
        ...
        model.add(Dense(1, activation='linear'))  # Our last layer doesn't need a non-linear activation function,
        # unless it is useful for the type of answer we want. The output layer should have the same number of neurons
        # as outputs you are generating. In this case, it is just producing one number.
